...
**Все блокеры исправлены. Можно приступать к реализации.**
...
#### Добавить поля для обезличивания финансовых данных:
...
### 1.2. 2-фазное удаление (Soft-delete → Hard-delete)
...
#### Фаза B: Полная зачистка (асинхронно, BullMQ worker)
...
**Дополнительно: Safety-net scheduler**
...
### 1.3. Защита от отправки пушей удалённым пользователям
...
### 1.4. Восстановление аккаунта (опционально)
...
1. Создать единый конфиг:

```typescript
// server/config/notifications.ts
export const NOTIFICATION_CONFIG = {
  horizonDays: 2, // Планируем на 2 дня вперёд (сегодня + завтра)
  // Планируем на 2 дня вперёд (сегодня + завтра)
  // Воркер раз в час, так что БД не раздуется
  minGapMinutes: 10, // Минимальный интервал между слотами (для большого количества уведомлений)
  // ВАЖНО: Для малого количества уведомлений интервал будет динамическим
  // (максимально отдалить, но равномерно распределить)
  jitterMinutes: 15, // Джиттер для естественности
  // ВАЖНО: minGapMinutes должен совпадать с параметром в preventSimultaneousNotifications()
};
```

2. Использовать конфиг везде:
   - `regenerate-slots.service.ts`
   - `scheduler.service.ts`

### 2.2. Исправление распределения слотов по дням
...
**Важно:** userId нужно передавать в `generateSlotTimes()`. Добавить параметр `userId: number`.

### 2.4. Улучшенный анти-оверлап
...
2. **Проверка пересечений перед созданием слота (опционально, для дополнительной защиты):**
   - Можно добавить проверку в `regenerate-slots.service.ts` перед `insertSlot()`
...
### 2.5. DB-ограничения как safety net
...
### 2.6. Регенерация: удалять queued тоже
...
2. **При генерации слотов:**

   - Ручные времена вставляются как есть (без проверки конфликтов перед вставкой)
   - Если при вставке возникает конфликт unique index (код 23505) - слот **пропускается** (не вставляется) с предупреждением в логах
   - Это означает: если два ручных времени попадают на одну минуту, второй не будет создан

3. **В `preventSimultaneousNotifications()`:**
...
- [ ] После удаления аккаунта ни один пуш больше не уходит (даже если job уже был enqueued)
- [ ] Удаление возвращает ответ быстро (<300ms, без ожидания каскада)
- [ ] Повторный вызов удаления идемпотентен
- [ ] Grace-period работает (можно восстановить в течение N дней)
- [ ] Финансовые данные обезличиваются, а не удаляются
- [ ] Все user-owned данные удаляются каскадом

### Генерация слотов:

- [ ] Сценарий "включил в 18:00, 5/день": сегодня планируется "сколько влезет до конца окна", завтра планируется ровно 5
- [ ] Нет >1 pending слота в одну и ту же минуту у пользователя (unique index предотвращает дубли)
- [ ] Ручные времена (`customSlotTimes`): при конфликте unique index слот пропускается (не вставляется) с предупреждением в логах. В UI показывается warning при сохранении пересекающихся времён, но сохранение разрешено.
- [ ] Unique index предотвращает дубли при параллельных инсертах
- [ ] При большом количестве уведомлений нет "кучи в 22:30"
- [ ] Минимальный интервал между слотами соблюдается (динамически: для малого количества - максимально отдалить, для большого - минимум 10 минут)
- [ ] Уведомления внутри одной группы равномерно распределены по дню (первое около начала окна, последнее около конца, остальные равномерно)
- [ ] Джиттер детерминирован (одинаковые входные данные → одинаковый результат)
...
1. **Приоритет 1 (критично):** Исправление генерации слотов
   ...
   - Распределение по дням (сегодня частично, завтра полностью)
   - Равномерное распределение внутри дня (первое около начала, последнее около конца)
   - Динамический интервал (для малого количества - максимально отдалить, для большого - минимум 10 минут)
   - Детерминированный джиттер
   - Улучшенный анти-оверлап (поиск в обе стороны)
   - DB-ограничения (partial unique index)
   - Warning для ручных пересекающихся времён

2. **Приоритет 2 (важно):** Удаление пользователя

   - Схема БД (поля + каскады)
   - 2-фазное удаление
   - Защита от пушей
   - Grace-period
...
1. **Добавление полей для удаления:**
...
- Добавить очередь `user-deletion`
- Добавить worker для обработки удаления
- Зарегистрировать `user-deletion-sweeper` в планировщике (cron ежедневно в 00:00 UTC)
...
1. **`generateSlotTimes()`** - добавить параметр `userId: number`
   ...
   - Обновить все вызовы в `regenerate-slots.service.ts`
