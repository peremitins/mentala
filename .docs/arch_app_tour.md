...
- Юзер **не может пропустить** тур через крестик / Esc / клик вне.
- Юзеру **доступны только кнопки «Назад» / «Далее»** в bubble. Все остальные UI-элементы заблокированы overlay'ем.
...
**Кейс «истёкший триал»:** у юзера нет доступа к чату → шаги `chat-mic`, `chat-realtime-voice`, `chat-summary`, `summary-example` помечены `skipIf: (ctx) => !ctx.hasChatAccess` → автоматически пропускаются. Тур переходит сразу с `home-hero` на `therapy-quick-chat`. Tap-анимация на «home-hero» при этом **не воспроизводится**, потому что в чат мы не идём.
...
watch(
  ...
  (canStart) => {
    if (canStart && !tour.isActive.value) {
      // отложенный старт через 600ms (даём странице отрендериться)
      pendingStartTimeout = setTimeout(() => tour.start(), 600);
...
Кнопки «Назад / Далее» в bubble — единственное, что доступно для нажатия.
...
### Шаг 1 — Главная (`/`) — карточка ИИ-помощника

- **Target:** `[data-tour="home-hero"]` (большая карточка ИИ-ассистента).
- **Bubble:**
...
### Шаг 3 — Чат — голосовой режим (трубка)
...
### Шаг 4 — Чат — кнопка «Подвести итог»
...
- **На «Далее»:** анимация тапа на эту же кнопку → `navigateTo('/session-summaries-user')` (на пример экрана итогов).

### Шаг 5 — Итог сессии (`/session-summaries-user` или mock-страница)

- **Target:** null (центрированный bubble без spotlight, либо подсветка всего блока с итогом).
- **Bubble:**
  - Заголовок: «Так выглядит итог»
  - Текст: «Здесь — три раздела: о чём была сессия, что было важным и какие следующие шаги. К итогам всегда можно вернуться через "Историю сессий" на главной.»
  - Медиа: скриншот примера.
- **На «Далее»:** `navigateTo('/therapy')` (без tap-анимации, т.к. целевой элемент — нижний таб, его подсветим в следующем шаге).

### Шаг 6 — Терапия (`/therapy`) — кнопка «Поговорить»

- **Target:** `[data-tour="therapy-quick-chat"]` (первая кнопка «Поговорить» в первой теме).
- **Bubble:**
  - Заголовок: «Поговорить»
  - Текст: «Запускает сессию с ИИ-помощником именно по этой теме. Он уже знает контекст и подготовлен работать с этим состоянием.»

### Шаг 7 — Терапия — кнопка «Медитация»

- **Target:** `[data-tour="therapy-quick-meditation"]`.
- **Bubble:**
  - Заголовок: «Медитации по теме»
  - Текст: «Подборка медитаций, специально подобранных под это состояние. Можно слушать прямо сейчас.»

### Шаг 8 — Терапия — кнопка «Дыхание»

- **Target:** `[data-tour="therapy-quick-breath"]`.
- **Bubble:**
  - Заголовок: «Дыхательные практики»
  - Текст: «Короткие упражнения, чтобы быстро снять напряжение. Работают за 2–5 минут, можно делать где угодно.»

### Шаг 9 — Терапия — провал в тему

- **Target:** `[data-tour="therapy-card"]` (вся карточка первой темы).
- **Bubble:**
  - Заголовок: «Каждая тема — отдельный мир»
  - Текст: «Внутри темы есть свои настройки напоминаний, медитации и дыхание. Давай заглянем внутрь.»
- **На «Далее»:** анимация тапа по карточке темы → `navigateTo('/therapy/anxiety')`.

### Шаг 10 — Тема (`/therapy/[key]`) — блок напоминаний
...
### Шаг 11 — Тема — тумблер напоминаний
...
### Шаг 12 — Тема — провал в настройки напоминаний

- **Target:** `[data-tour="therapy-reminders-edit"]` (кнопка «Настройки напоминаний»).
- **Bubble:**
  - Заголовок: «Тонкая настройка»
  - Текст: «Здесь подберёшь оптимальную частоту, временной диапазон и дни. И главное — напишешь личные пожелания.»
- **На «Далее»:** анимация тапа → `navigateTo('/therapy/anxiety/notifications')`.

### Шаг 13 — Настройки напоминаний — общий обзор
...
### Шаг 14 — Настройки напоминаний — «Мои пожелания»

- **Target:** `[data-tour="custom-prompt"]` (блок «Мои пожелания»).
- **Bubble:**
  - Заголовок: «Персонализация под себя»
  - Текст: «Опиши в свободной форме, как ты хочешь получать напоминания: тон, стиль, какие-то детали. ИИ адаптирует тексты под твои пожелания.»
...
|---|---|---|
| `data-tour="home-hero"` | `app/pages/index.vue` | Карточка ИИ-ассистента (section) |
| `data-tour="home-hero-cta"` | `app/pages/index.vue` | Кнопка «Начать» в карточке |
...
| `data-tour="therapy-quick-chat"` | `app/components/notifications/NotificationIndexPage.vue` | Кнопка «Поговорить» |
| `data-tour="therapy-quick-meditation"` | `app/components/notifications/NotificationIndexPage.vue` | Кнопка «Медитация» |
| `data-tour="therapy-quick-breath"` | `app/components/notifications/NotificationIndexPage.vue` | Кнопка «Дыхание» |
...
| `data-tour="therapy-reminders-edit"` | `app/components/notifications/NotificationsSummaryCard.vue` | Кнопка «Настройки напоминаний» |
| `data-tour="custom-prompt"` | `app/components/notifications/NotificationSettingsPage.vue` | Section «Мои пожелания» |
