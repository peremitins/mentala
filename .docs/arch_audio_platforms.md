...
- Громкость аппаратными клавишами: работает на iOS (любой браузер — движок WebKit, через `navigator.audioSession`), desktop, в нативном Android-аппе (Java media-режим) И на Android в браузере/PWA. **Ключ к Android-web:** звук ассистента проигрывается через Web Audio (`AudioContext`, `latencyHint:'playback'`) → медиа-канал `STREAM_MUSIC`, а НЕ через WebRTC-`<audio>`-элемент (его Chrome при активном микрофоне рендерит на тихом call-канале → earpiece, и клавиши его не двигают). На `STREAM_MUSIC` аппаратные клавиши управляют громкостью нативно. Прежнее ограничение (клавиши не работали на Android-web; `navigator.audioSession` в Chrome нет — caniuse, jitsi-meet#16020) снято именно переносом звука на Web Audio.
- Ручной регулятор громкости (`AssistantVolumeControl.vue` — иконка в `PageHeader` слот `#actions` + слайдер) **удалён**: это был костыль для Android-web, где клавиши не работали. Теперь работают → регулятор не нужен. `realtimeVoiceUi` хранит только статус сессии (для оверлеев/ambient-рамки); слот `#actions` в `PageHeader` тоже убран. Удаление чисто клиентское — API/DTO/БД не затронуты.
- Усиление громкости (потолок source тихий): `remoteAudioElement.volume` упирается в 1.0, поэтому **только на Android web/PWA** выход идёт через Web Audio: `remoteAudioElement (muted, kick) → MediaStreamSource → GainNode (фикс ×MAX_OUTPUT_GAIN, сейчас ×3) → DynamicsCompressor(brickwall-лимитер) → destination`. Усиление **фиксировано** на ×MAX — живую регулировку отдаём аппаратным клавишам поверх `STREAM_MUSIC`. muted-элемент обходит баг Chrome (тишина `createMediaStreamSource` на remote-потоке — подтверждённый muted-element kick паттерн). При любой ошибке — автоматический откат на прямое воспроизведение элемента (потолок 1.0), звук не пропадает. **На нативном Android Web Audio НЕ применяется** — там маршрут/громкость держит Java-плагин (`MODE_NORMAL` + `STREAM_MUSIC`), а звук идёт через обычный WebRTC-элемент; Web Audio поверх ломал управление клавишами (звук уходил с канала клавиш, неуправляемо громкий ×3) и перегружал AEC/глитчил playback (модель «слышит себя»). iOS/desktop тоже на прямом пути. Включается флагом `enableOutputGainBoost` в `transport.start()` (composable: `shouldBoostOutputGain()` — `getPlatform()==='web'` И Android UA; нативные iOS/Android исключены).
- **Лимитер — brickwall ТОЛЬКО у потолка** (`threshold ≈ -1 dBFS`, `ratio 20`, `knee 0`): прежний мягкий лимитер (`-6 dB`, `12:1`) сжимал всё громче -6 dB и нивелировал прирост — смена ×1.5→×2.5 не давала разницы на слух, плюс «пампинг» = «тихо и плохое качество». Теперь усиление слышно почти линейно, лимитер ловит только клиппинг на пиках.
- **Маршрут вывода**: `outputAudioContext` создаётся с `latencyHint: 'playback'` — это просит «медиа»-путь (Android → `STREAM_MUSIC`, громкий динамик), а не низколатентный, который при активном getUserMedia Chrome может цеплять к разговорному каналу (earpiece, тихо). Корень «тихо на Android-вебе» — именно ОС-маршрут на earpiece/voice-call на части устройств (Samsung/Infinix), а не уровень сигнала: усиление сигнала маршрут не перебивает. `setSinkId` для output на Android Chrome ненадёжен, `echoCancellation` там не отключается (issues.webrtc.org#42233375).
- **Диагностика на устройстве** (`startOutputAudioDiagnostics`): RMS-зонд после gain логирует `graphAudible`/`graphMaxRms` (звучит ли граф вообще, или это cross-origin тишина) + состояние контекста + список `audiooutput`-устройств. Лог-префикс `[RealtimeVoiceTransport] Output gain diagnostics`. Однозначно отделяет «граф молчит → нужен другой источник» от «громко в графе, но маршрут тихий → нужен другой выход».
