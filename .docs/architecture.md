...
• `useSceneAudio` управляет воспроизведением, fade‑in/out и таймером выключения при уходе в фон; при уходе в фон звук продолжается только если задан `backgroundPlayMinutes` (иначе останавливается), при возвращении в foreground пытается восстановиться (visibilitychange/AppState). Loop-сцены играют через WebAudio (включая iOS) для бесшовности; HTMLAudio остаётся fallback для non-loop/ошибочных сценариев, а старт HTML ждёт минимальную готовность (`loadedmetadata/canplay`) вместо обязательного `canplaythrough`.
