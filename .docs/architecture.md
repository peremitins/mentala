...
• `entity_key` — единое поле для идентификации источника уведомлений. Для кастомных сущностей используется slug, для готовых шаблонов - ключ шаблона.
...
• `ai` — Способ создания с помощью AI (OpenAI GPT)
...
• В настройках пользовательской привычки скрыты блоки «Фокус уведомления» и «Стиль уведомлений», вместо этого доступен новый блок «Тексты уведомлений» (до 100 текстов, каждый ≤ 178 символов, поддерживается плейсхолдер `{name}` и анимации при добавлении/удалении).
...
• Планировщик (`scheduler.service.ts`) и тестовая отправка сначала проверяют `meta.customTexts`: если массив задан и `textSource === 'templates'`, текст берётся из него (равномерный случайный выбор) и попадает в слот с templateId `custom_user_text`; при `textSource === 'ai'` или `'hybrid'` используются AI-генерированные тексты; только при отсутствии пользовательских текстов и AI-текстов происходит подбор шаблонов.
...
• Сохранение настроек (`/api/notifications/prefs/therapy`) кладёт кастомные тексты в `meta.customTexts`; планировщик и тестовая отправка обрабатывают терапию так же, как привычки: при `textSource === 'templates'` и наличии кастомных текстов шаблоны не подбираются, а в слоты попадает текст пользователя и templateId `custom_user_text`; при `textSource === 'ai'` или `'hybrid'` используются AI-генерированные тексты.
