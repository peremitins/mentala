...
• Реализована локализация Support-страниц в двух языках (RU/EN) через `vue-i18n`: словари `apps/landing/i18n/support/ru.ts` и `apps/landing/i18n/support/en.ts`; эти же словари подключены в `app/plugins/i18n.ts` для страницы `/support` в приложении.
