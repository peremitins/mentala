...
• Подписочный usage-инвариант: weekly AI usage считается по окну `max(startOfWeek, startOfCurrentAccessPeriod)`. `startOfCurrentAccessPeriod` берётся из `user_subscriptions.start_date` для активного paid-плана либо из `users.nextChargeAt` для scheduled/past*due доступа без активной paid-подписки. Это не даёт минутам из trial или предыдущего paid-периода переноситься в новый тариф, если смена доступа произошла посреди недели; клиентский `subscription` store после наступления `trialEndsAt`, `nextChargeAt`, `scheduledChange.effectiveAt` или `graceEndsAt` обязан рефетчить subscription/usage и инвалидировать usage-кэш при смене access-period.
• Realtime Voice subsystem реализован как отдельный bounded context, изолированный от текстового чата. Бэкенд-контур собран в `server/application/realtime/*` (`access`, `quota`, `instructions`, `session`) + provider adapter `server/infrastructure/llm/openai-realtime.ts`и`/api/realtime/session|call|session/event|session/end`; voice использует актуальный OpenAI Realtime WebRTC contract в unified server-proxy варианте: `/api/realtime/session`только создаёт продуктовую voice-session, строит OpenAI session-config и кладёт его во временный server-side store, а сам SDP-handshake к`/v1/realtime/calls` выполняется через BFF (`/api/realtime/call`) под обычным server API key. Если включён `AI_RELAY_URL`/`AI_USE_RELAY`, основной RU-backend не должен ходить в OpenAI Realtime напрямую: `openai-realtime.ts`отправляет подписанный JSON-запрос в`apps/ai-relay`по тому же HMAC-паттерну, что и`Responses API`, а уже relay собирает multipart `sdp + session`и обращается к`https://api.openai.com/v1/realtime/calls`; direct-вызов в OpenAI остаётся только fallback-веткой для окружений без relay. Это убирает зависимость старта voice-сессии от внешнего `client_secrets`шага и делает mobile/web одинаковыми по инициализации. Proxy обязан пересылать raw SDP без`trim()`и без модификации завершающих CRLF, иначе OpenAI может отвергнуть offer как`invalid_offer`. Сам `/api/realtime/call`не должен зависеть от обычной app-session: после успешного`/api/realtime/session`сервер выпускает короткоживущий signed handshake-token, привязанный к`sessionId`/`userId`, и только он используется для допуска к proxy-handshake; при этом web-клиент обязан передавать стандартный `X-CSRF-Token`в ручной handshake-fetch, потому что запрос идёт по cookie-каналу мимо общего`$api`клиента. Отдельный hard quota`60 минут`на quota-period остаётся, voice дополнительно расходует existing weekly AI minutes через`therapy_sessions`, имеет server-side `kill switch` (`FEATURE_REALTIME_VOICE_ENABLED`) и принудительно закрывает stale/conflicting realtime sessions. Для mobile reliability session-config обязан включать OpenAI `input_audio_noise_reduction: near_field`и отключать provider-side eager interrupt (`turn_detection.interrupt_response = false`), а клиентский barge-in на iOS/Android должен быть консервативным: не слать `response.cancel`, пока реально играет assistant playback, иначе WebView легко ловит собственный динамик как новый user turn. Клиентский idle auto-stop для voice должен оставаться включённым, но не может быть слепым wall-clock таймером: countdown обязан сбрасываться не только по provider events, но и по реальному микрофонному сигналу выше шумового порога, иначе длинная непрерывная реплика пользователя будет ошибочно завершать voice-session с `timeout`. Постоянные realtime `session.instructions` не должны безусловно содержать детальный crisis-блок (`mentalHealthPack.crisisProtocol` / `ВНИМАНИЕ: ПОТЕНЦИАЛЬНЫЙ КРИЗИС`): такой блок допустим только как явная условная safety-инъекция, иначе весь voice-канал начинает вести себя так, будто пользователь уже в кризисе. Фронтенд-контур вынесен в`app/services/realtime/\*`+`app/composables/useRealtimeVoiceSession.ts`: transport/controller отделены от `app/pages/index.vue`, browser capability resolution централизована в `realtimeVoiceBrowser.ts`с fallback на legacy`webkit/moz`WebRTC и`getUserMedia`, а `http://localhost`/`127.0.0.1`для Capacitor/WebView трактуются как доверенный origin для проверки secure-context; transcript идёт только в runtime`chat.messages`как`transient`-сообщения без server persistence, а `RealtimeVoiceChatAdapter`жёстко гарантирует один assistant bubble на один`response_id`, резервирует user bubble уже на `input_audio_buffer.speech_started`/`conversation.item.created`для сохранения порядка `user -> assistant`, не создаёт assistant bubble на пустом `response.output_item.added`, оставляет feedback включённым для финальных assistant realtime сообщений, фильтрует пустые realtime bubbles и делает финализацию по final transcript. При переходе voice-сессии в `active` клиент один раз проигрывает локальный cue из `public/chat_on.wav`; основной путь — заранее разблокированный `AudioContext` с preloaded/decode-буфером, fallback — `HTMLAudioElement`, чтобы короткий сигнал стабильно работал в web и Capacitor WebView на iOS/Android. При любой ошибке старта/соединения connecting-tone обязан немедленно останавливаться, а UI не должен рендерить raw realtime error inline под composer: пользователю показывается понятный toast. Одновременно UI показывает transient assistant bubble с коротким приглашением начать говорить; этот bubble не участвует в feedback, удаляется при первом `speech_started` либо при teardown voice-сессии и обязан использовать addressing-aware copy из `app/lib/addressingCopy.ts`, чтобы formal/informal в voice и текстовом чате не расходились.
• Пока активна или запускается realtime voice-сессия, обычная кнопка диктовки `speech-to-text`в`app/pages/index.vue`должна быть жёстко disabled и визуально выглядеть недоступной; смешивать realtime transport и локальную voice-dictation в одном composer нельзя, иначе легко получить гонки за микрофон и нестабильное состояние UI.
• Chat composer для realtime voice больше не использует отдельную верхнюю explanatory-плашку. Вход в voice находится прямо в нижнем action-row между обычной диктовкой и`send` как отдельная call-кнопка (`idle / connecting / active / stopping`), а UX-подсказки сведены к состоянию самой кнопки и только к компактному inline-error под composer. Кнопка использует короткий локальный connecting-tone через `AudioContext`и haptic feedback через`@capacitor/haptics`/`navigator.vibrate`, при этом tone живёт только в фазе `starting`и гарантированно останавливается при`active / idle / error / dispose`.
• Настройка голоса ассистента и его гендерной персоны должна иметь один source of truth в `chat_settings.assistant_voice`. `GET /api/user/me`возвращает только derived bootstrap-проекцию`assistantSettings`, но не является write-source. Категория `женский/мужской`является product-level классификацией из shared voice catalog, а не отдельным persisted полем; derived`assistantGender`обязан одинаково использоваться и в обычной TTS-озвучке, и в Realtime session config, и в prompt/instructions для`Responses`/`Realtime`, чтобы audio-голос и текстовая self-reference ассистента не расходились.
• Shared voice catalog локализован на уровне presentation-слоя (`ru`/`en`) и больше не показывает провайдерские ids вроде `Shimmer/Echo`пользователю. Для`ru`женский сегмент упорядочен как`Вера / Надежда / Любовь` (`shimmer / sage / coral`), мужской стартует с `Алексей` (`echo`). Глобальный дефолтный voice для новых `chat_settings`—`echo`, а fallback внутри женского сегмента — первый каталоговый voice `shimmer`.
• Voice-picker в `/settings/assistant`намеренно минималистичен: показываем только локализованные имена голосов внутри выбранного сегмента`Женский / Мужской`, без preview-кнопок, без описаний тембра и без дополнительных explanatory-текстов; карточки голосов визуально синхронизированы с `AssistantToneGrid`и используют те же glass-state стили активного/неактивного состояния и ту же вертикальную плотность, без продуктового`primary`.
• Контракт chat settings для этой подсистемы закреплён через shared Zod DTO (`shared/dto/chat-settings.ts`): `/api/settings/chat`принимает и возвращает`assistantVoice`только из curated каталога пересечения`Realtime + TTS`, не делает silent fallback при записи и опционально отдаёт derived `assistantVoiceMeta`. Для клиентских voice-related direct fetch (`useTTS`, keepalive-завершение realtime) используется единый runtime baseURL-resolver `app/utils/runtime-api.ts`, синхронизированный с `app/plugins/api.ts`, чтобы preview и voice-запросы работали одинаково в web, Capacitor local bundle (`localhost`) и external dev-server сценариях.
...
• Global addressing (`user_preferences.addressing`) является сквозным слоем персонализации для чата и части продуктового UI: значение протягивается через `/api/chat`и`/api/chat/stream`, участвует в сборке prompt-контекста (`buildDeveloperContext`, `buildWelcomePrompt`, crisis guidance, alternative openings), а на клиенте приходит в `/api/user/me`и используется из`auth.user.addressing`для chat/welcome/habits/therapy/gratitude-diary экранов. На этом этапе`welcome_prompts`в БД не дублируются по addressing: вариативность накладывается кодом поверх DB-backed prompt content.
• Базовые modal/select примитивы в приложении должны работать на одном layer-manager. Для диалогов и select-ов используем`reka-ui`; смешивание `reka-ui`и`radix-vue`в одной focus-trap цепочке приводит к гонкам`DismissableLayer`, `aria-hidden` warning и залипанию outside pointer-events при вложенных popup-слоях.
...
• iOS‑гайд и паритет с Android: см. `.docs/IOS\*SETUP.md`(dev/prod, push, Apple Developer Program, FCM/APNs особенности).
...
• Пользовательские названия тарифов локализуются только через i18n и общий хелпер`app/utils/planI18n.ts`: для `ru`UI обязан показывать кириллические`Базовый / Про / Премиум`и составные лейблы вроде`Про и Премиум`, не выводя `Basic / PRO / Premium` напрямую в компонентах, paywall-модалках и карточках подписки.
...
• Mobile dev-flow держится на одном фиксированном Nuxt-порту `3000`. `pnpm dev` не должен молча стартовать второй экземпляр на `3001`, если `3000` уже занят: тогда `cap:sync:device` и установленное mobile-приложение останутся привязаны к `http://<LAN_IP>:3000`, а фронтенд окажется на другом порту. Для этого entrypoint dev-режима сначала проверяет listener на `3000`: если там уже живой `nuxt dev` этого проекта, второй экземпляр не стартует; если порт занят чужим процессом, команда падает с явной ошибкой. `scripts/setup-capacitor-dev.sh` перед патчем `server.url` дополнительно проверяет доступность `http://127.0.0.1:3000`, а при подключённом Android-устройстве по `adb` после `cap sync` автоматически пересобирает и переустанавливает debug APK, чтобы телефон не оставался на старом `server.url` после смены live-reload-конфига.
• Для real device `cap:sync:device` не вводит новые публичные env и маршрутизирует платформы по-разному: iOS остаётся на LAN-входе через локальный Caddy reverse proxy `http://<LAN_IP>` (с fallback на прямой `http://<LAN_IP>:3000`), а Android при подключённом `adb` всегда получает `http://localhost:3000` + `adb reverse tcp:3000 tcp:3000`. Это нужно не только для VPN/локальных сетевых фильтров на Mac, но и для secure-context инварианта realtime voice: в Android WebView микрофон/WebRTC не должны зависеть от небезопасного `http://192.168.x.x` origin.
• Обновление от 19 марта 2026 (Android realtime audio routing): `RealtimeVoiceTransport` использует `HTMLAudioElement` как основной playback-path и best-effort выставляет `navigator.audioSession.type = 'playback'` на поддерживаемых WebView. Для Android поверх этого есть узкий нативный bridge `MentalaRealtimeVoiceAudio`: он не переводит realtime voice в `MODE_NORMAL`, потому что это ломает второй и последующие user-turn в WebRTC duplex-сценарии. Вместо этого bridge удерживает `MODE_IN_COMMUNICATION`, пинует `communication device` на `built-in speaker` (`setCommunicationDevice`, на старых API fallback через `setSpeakerphoneOn(true)`), но hardware volume keys направляет в `STREAM_MUSIC`, потому что удалённый assistant playback реально приходит как `USAGE_MEDIA`, а не как `USAGE_VOICE_COMMUNICATION`. Перед стартом и на каждом `output_audio_buffer.started` JS повторно вызывает этот bridge, чтобы переживать Chromium/WebView route resets без потери микрофона. Важный нюанс Capacitor 7: локальный plugin, зарегистрированный через `MainActivity`, должен быть добавлен до `super.onCreate(...)`, иначе Bridge создаст `PluginHeaders` без него, и JS будет получать `"plugin is not implemented on android"`. Поэтому JS-слой realtime voice не должен опираться на `Capacitor.isPluginAvailable()` для этого bridge; Android runtime должен пытаться вызвать plugin напрямую и считать недоступность только по фактическому reject/error.
• Обновление от 19 марта 2026 (Android realtime voice background): для lock-screen сценария добавлен отдельный foreground service `MentalaRealtimeVoiceForegroundService` с типами `microphone|mediaPlayback` и `PARTIAL_WAKE_LOCK`. Он стартует только на время активной realtime voice-сессии через `app/services/realtime/realtimeVoiceForegroundBridge.ts`, не переиспользуется медитациями и не меняет audio route. Задача слоя — best-effort удержать process/CPU и доступ к микрофону после выключения экрана, не ломая текущий speaker/volume fix. Это не полноценный native-call стек и не гарантирует идеальный reconnect/resume после произвольного background-suspend, но закрывает именно сценарий «экран заблокировался во время уже активного realtime-диалога».
• Chat UI: блок `SuggestedChips` на `app/pages/index.vue` должен скрываться на время активного `realtime voice` (включая `starting/stopping`), чтобы в voice-режиме внизу чата не оставались интерактивные quick-reply чипсы, конфликтующие с заблокированным текстовым вводом.
• В `app/components/settings/SubscriptionBlock.vue` блок управления подпиской показывает два независимых usage-индикатора: недельные AI-минуты из `/api/subscriptions/usage` и отдельный лимит голосового диалога (`used / limit / remaining / resetsAt`) из того же ответа. Второй блок визуально отделён верхней границей и отступом, чтобы usage-метрики не слипались. Это сделано через расширение `subscriptionStore.usage`, чтобы экран настроек не выполнял отдельный запрос только ради `realtime voice` квоты.
• `Realtime Voice` quota считается только в секундах. Для `server/application/realtime/realtime-voice-quota.service.ts` авторитетным считается точный посекундный usage (`limitSeconds / usedSeconds / remainingSeconds`) внутри quota-period; именно он ограничивает старт сессии и `maxDurationSeconds`. Legacy-минутные поля из realtime payload/store удалены полностью, чтобы не держать две параллельные модели учёта.
• Дефолтная OpenAI-модель для realtime voice в `server/config/realtime.ts` — `gpt-realtime-mini`. Переопределение через `OPENAI_REALTIME_MODEL` остаётся возможным, но базовый серверный fallback должен быть экономичным, а не `gpt-realtime`.
...
• API-клиент в`app/plugins/api.ts`использует единую кроссплатформенную стратегию`baseURL`: на web берётся `NUXT_PUBLIC_API_SERVER_URL`; на native в dev, когда приложение реально поднято с внешнего `server.url`, источником API становится `window.location.origin`, чтобы live reload на реальном устройстве и эмуляторе шёл через тот же dev-server origin.
• Резолв медиа (`app/utils/media.ts`+`app/utils/media-base.ts`): в `dev`для native runtime приоритет у `window.location.origin`, когда приложение поднято с внешнего `server.url`; в `production`приоритет остаётся у`NUXT_PUBLIC_MEDIA_BASE_URL`.
...
• Реализована публичная Support-страница на лендинге: `https://mentala.app/support` (`apps/landing/pages/support.vue`) с включением маршрута в prerender (`apps/landing/nuxt.config.ts`).
• Реализована отдельная Support-страница внутри основного приложения: `/support` (`app/pages/support.vue`), переход на неё доступен из `/settings` без редиректа на лендинг.
• Локализация Support-страниц в двух языках (RU/EN) унифицирована в общей структуре i18n: `i18n/locales/{ru,en}.ts` + `i18n/rules/{numbers,pluralization}.ts` + `i18n/index.ts` (и в приложении, и в лендинге). Ключи верхнего регистра используются как группировка разделов/страниц (`SUPPORT`, `NOTIFICATIONS`), а правила множественного числа для RU подключаются через `pluralRules` в `vue-i18n`.
• Лендинг (`apps/landing/pages/index.vue`) полностью переведён на `vue-i18n` через namespace `LANDING`: UI-контент, модалки и формы, массивы контента в `script setup`, SEO/OG/Twitter и JSON-LD. Единая логика определения/переключения локали вынесена в `apps/landing/composables/useLandingLocale.ts` и используется в `/` и `/support` (query `lang|locale` → cookie `mentai.lang` → browser fallback).
• Переключатель языка в лендинге вынесен из хедера в фиксированный floating-блок (`position: fixed`) в левом нижнем углу на страницах `/` и `/support`; визуально используется единый компонент `apps/landing/components/ui/LanguageSelect.vue` (single-select, стиль совпадает с селектом в waitlist-модалке).
• UX Support-страниц усилен под App Review: явная CTA-кнопка `mailto` «Написать в поддержку», кликабельные ссылки на инструкции отмены Apple/Google, явное указание `https://my.mentala.app` для web-управления подпиской, уточнение refund-процесса (какие данные прислать + ориентир по сроку рассмотрения).
• В пользовательских каналах поддержки канонический контакт обновлён на `support@mentala.app` (лендинг-футер + structured data + public legal pages), а ссылка на `/support` доступна и в лендинге, и в `/settings` приложения.
• Правило синхронизации: любые текстовые/юридические изменения в Support-контенте должны вноситься одновременно в обе реализации (`apps/landing/pages/support.vue` и `app/pages/support.vue`) в рамках одного PR/коммита.
• Настройки (IA v2):
• `/settings` — главный список (профиль, подписка, ассистент, уведомления, конфиденциальность, поддержка, документы, приложение, служебное).
...
• Блок «Поддержка» выделен отдельно от «Документов»: содержит вход в `/support` (вопросы, отмена подписки, возвраты).
• Юридические документы: статические HTML в `public/legal/` с локалями (`terms-of-service-ru.html`, `terms-of-service-en.html`, `privacy-policy-ru.html`, `privacy-policy-en.html`); ссылки в auth/settings/support/landing выбираются по текущему языку интерфейса (`ru|en`).
...
• Архитектурный вектор единой навигации (draft, без полной реализации): канонический документ — `.docs/unified_navigation_tz.md`. Source of truth для переходов из push, chat и внутренних рекомендаций должен стать typed `AppNavigationTarget`; `deepLink`, `data.action`, chip action и route-path рассматриваются как transport/adapters. Для LLM-assisted сценариев вводится отдельный слой `NavigationIntentExtraction`, но финальный target обязан выбирать deterministic resolver по whitelist-registry; модель не является source of truth для routing/paywall/fallback и не должна получать полный registry через длинный prompt. Для чата переход допускается только по явному клику пользователя; redirect на `/` для платных destination-ов признан неверным и подлежит замене на paywall flow с сохранением target context.
...
• Верхняя строка паттерна в `BreathPracticePlayer` рендерится по сегментам и синхронизируется с `phaseIndex` из `useBreathPracticePlayer`: активная фаза подсвечивается белым, соседние разделители чуть усиливаются, а для кастомных строк сначала используется текст из `practice.pattern` с fallback на генерацию из `practice.phases`.
...
• Горизонтальные ленты в каталогах медитаций и дыхательных практик стандартизованы через общий `app/components/ui/HorizontalScroller.vue`: на desktop добавляются динамические стрелки при скрытом контенте, wheel→horizontal scroll, drag мышью с `grab/grabbing` и edge-fade; на touch-устройствах сохраняется нативный свайп без лишних контролов. Автоматический стартовый scroll-hint отключён, чтобы не дёргать первый ряд при входе на экран.
...
• Память текстового чата (LLM) теперь разделена на три слоя. `chat_session_memories` хранит session-scoped chain/runtime compact-state только для активной `therapySession`; `therapy_session_messages` хранит временный encrypted transcript; `session_summaries` хранит handoff-summary только последней завершённой сессии; отдельная таблица `user_memory_profiles` хранит долгую память пользователя (`durable profile memory`) с устойчивыми фактами и предпочтениями. Для `durable profile memory` больше не применяются hard caps по числу элементов, длине строк и внутреннему char-budget: runtime сохраняет полный JSON profile, который вернула модель после normalizer/dedupe. Старый `user_response_ids` считается legacy и в runtime текстового чата больше не используется.
• Lifecycle памяти текста: внутри активной `therapySession` используется только session-scoped `previous_response_id` (Responses API) + `truncation: auto`; при завершении `therapySession` server-side lifecycle сначала сбрасывает chain, затем ставит BullMQ job `chat-session-summary`, которая одним LLM-вызовом строит и handoff-summary для следующей сессии, и обновляет долгую `durable profile memory`, после чего очищает временный transcript/runtime-state. Если очередь/Redis недоступны, summary pipeline обязан перейти на inline fallback, чтобы handoff не зависел от BullMQ. Клиентский `/api/session/finish` больше не является каноническим источником summary.
• Старт text `therapySession` теперь дополнительно привязан к клиентскому `chatSessionId`: `POST /api/therapy/session/start` переиспользует активную session только для того же `chatSessionId`, а новый `chatSessionId` считается новым lifecycle и server-side закрывает предыдущую активную session перед созданием новой. Это убирает скрытое продолжение старой chain после reload/повторного входа в чат, если best-effort client-end не успел отработать.
• Переключение между `Responses API`-чатом и `Realtime Voice` делается не через перенос raw API-state, а через явный sync handoff. Endpoint `/api/session/handoff` синхронно закрывает source-session, строит `handoff summary` из server-side transcript и сохраняет его в `session_summaries`; после этого target-mode стартует как новая session с подмешанным handoff. Для обычного завершения voice-session без mode switch используется тот же session-end summary lifecycle через `handleTherapySessionEnded`. Клиент realtime voice дополнительно зеркалит закрытие связанной `therapySession` отдельным вызовом `/api/therapy/session/end` после stop/page-leave: каноническое закрытие по-прежнему делает realtime/handoff endpoint, а mirror-call нужен для одинакового client lifecycle и наблюдаемости с текстовым чатом.
• `Realtime Voice` не использует `previous_response_id`, поэтому text-only guard по chain/tokens сам по себе его не защищает. Для live voice-сессии применяется отдельный runtime compaction по тем же бюджетам, что и в текстовом чате (`CHAT_MEMORY_SOFT_INPUT_TOKENS`, `CHAT_MEMORY_MAX_INPUT_TOKENS`, `CHAT_MEMORY_MAX_TURNS_PER_CHAIN`): после `response_completed` сервер считает usage/turns, при достижении порога строит `runtimeCompactState`, клиент отправляет в OpenAI `session.update` с обновлёнными `instructions`, затем удаляет старые `conversation.item.*` из live Realtime conversation и продолжает диалог уже поверх compact-state. При этом UI-история чата не очищается. Structured compaction state для voice нельзя строить на самой realtime-модели (`gpt-realtime-*`), потому что она не поддерживает `text.format=json_schema` в нашем Responses-based compaction pipeline; для этого шага runtime обязан переключаться на обычную structured-output-capable text model. Если compaction generation упала или вернула пустой state, chain нельзя сбрасывать и cursor нельзя двигать — runtime должен оставить текущий state как есть и попробовать compaction позже, а не “успешно” обнулить контекст. Generic realtime `error`-события после `session.update` / `conversation.item.delete` нужно считать recoverable по умолчанию: клиент не должен автоматически завершать voice-session и уходить в `handoff/provider_error`, пока сам WebRTC/datachannel не перешёл в `failed|closed`. Дополнительно baseline realtime session config обязан включать provider-side `truncation` (`token_limits.post_instructions = CHAT_MEMORY_SOFT_INPUT_TOKENS`, `retention_ratio < 1`) как safety net, но именно наше server/client compaction-плечо остаётся каноническим механизмом экономии токенов и переноса контекста внутри длинной voice-сессии.
• Для длинных активных сессий реализован runtime token guard: `SOFT_INPUT_TOKENS = 10000`, `MAX_INPUT_TOKENS = 15000`, `MAX_TURNS_PER_CHAIN = 24`. `turn` считается только как один user-request с одним основным assistant-response; retries, chips и summary pipeline в лимиты chain не входят. Приоритет guard-ов: `explicit_end/new_session_started` > `MAX_INPUT_TOKENS` > `MAX_TURNS_PER_CHAIN` > `SOFT_INPUT_TOKENS`; forced compaction никогда не включает текущее новое user-message в compaction prompt, это сообщение становится первым input новой chain.
• Prompt-assembly для текстового чата разделён на `session-bootstrap` и `per-turn`: на первом ходе chain отправляются `system` + стабильный developer bootstrap + memory blocks (`durable profile memory`, `handoff summary` или `runtime compact state`), на последующих ходах chain отправляется только `per-turn` developer context и текущее user-message. Это убирает бесконечное дублирование стабильных prompt-слоёв поверх уже длинной chain.
• `durable profile memory` хранится в short JSON с читаемыми ключами `schemaVersion/name/facts/preferences/context`. Она подмешивается не в каждый turn, а только в bootstrap новой cross-session chain / voice-session; если внутри той же text-session произошёл runtime compaction, при повторном bootstrap используется только `runtimeCompactState`, без повторной подмеси `user_memory_profiles`, чтобы не платить за один и тот же durable context повторно. Слой хранит только устойчивые explicit факты пользователя и долгие предпочтения; временные эмоции, одноразовые детали и служебные safety-state туда не попадают. Обновление теперь полностью model-driven: `finish_session` получает previous profile как контекст, анализирует весь transcript завершённой сессии и возвращает уже готовый канонический `profile`, который сервер сохраняет как есть после shape-normalization. Сервер больше не делает semantic `add/replace/delete` merge между старой и новой памятью и не принимает решения о противоречиях, приоритетах и вытеснении элементов; вся эта логика вынесена в prompt и structured output модели. Для контроля бюджета previous profile передаётся в модель отдельной budgeted prompt-projection с потолком `<=1000 chars`, а сам ответ модели ограничен strict schema: `facts<=3`, `preferences<=3`, `context<=2`, `name<=40 chars`, `item<=80 chars`. Session-end summary и durable profile update обязаны строиться по всему transcript завершённой `therapySession`, а не только по хвосту после последнего runtime-compaction cursor; иначе ранние факты из длинной сессии теряются и в `user_memory_profiles` попадает только недавний фрагмент разговора. Исключение только для prompt-проекции длинных `realtime voice` сессий: если внутри voice-сессии уже есть `runtimeCompactState`, то в `session_end_memory_bundle` ранние `ASSISTANT` turns до cursor можно не передавать дословно, но все `USER` turns по-прежнему должны попадать в анализ целиком, а смысл ранних assistant-ответов должен приходить из `runtimeCompactState`. Это режет стоимость `finish_session`, не теряя ранние user-факты. Дополнительно transient transcript активной текстовой сессии больше нельзя собирать только append-логикой по последнему pair: после каждого успешного chat-turn runtime обязан пересобрать `therapy_session_messages` из полного клиентского `messages` history плюс нового ответа ассистента, чтобы session-end summary переживал dev-reload, смену encryption-mode и любые расхождения локального session transcript с текущей клиентской историей. `finish_session` запрос к Responses API должен анализировать не набор отдельных последних items, а явный сериализованный transcript всей сессии и использовать strict `text.format=json_schema`; иначе handoff/profile update деградируют к пустому fallback JSON или к summary по хвосту диалога. `session_summaries` должна записываться после каждого завершения сессии с нормализованным handoff payload, даже если он пустой: отсутствие meaningful summary не должно ломать lifecycle и скрывать факт, что session-end pipeline отработал. При этом сам `handoff summary` теперь хранит только session-specific фон для следующей сессии: summary/темы/паттерны/полезные интервенции/unfinished threads/risk/guidance. Имя, устойчивые facts, preferences и long-term context не должны дублироваться в `handoff` и передаются только через `user_memory_profiles`; при prompt-assembly `handoff` дополнительно сериализуется без пустых полей, чтобы не раздувать bootstrap-контекст. Debug/plaintext режим (`SUMMARY_ENCRYPTION_DISABLED=true`) должен читаться тем же decode-path, что и encrypted payload: пустой `memory_iv` не делает запись невалидной, иначе previous profile теряется и следующий session-end ошибочно строит память “с нуля”. Session-end parser также обязан принимать nested `profile`/`handoff` и совместимый flat top-level payload, если модель слегка отклонилась от формата ответа; иначе `user_memory_profiles` и `session_summaries` могут внешне “обновляться” непредсказуемо или не обновляться вовсе. Для session-end profile update больше не задаётся явный `max_output_tokens`; вместо этого размер model-output ограничивается strict schema (`maxItems` / `maxLength`). Runtime при этом остаётся backward-compatible к старым compact-ключам `v/n/f/p/g` при чтении profile из БД и при парсинге совместимого full-profile ответа модели.
• Summary storage, durable profile memory, runtime compact-state и transcript versioned (`schemaVersion`) и по умолчанию хранятся в encrypted виде через `SUMMARY_AES_KEY` (AES-256-GCM). Для локального dev-debug все эти memory-payload paths могут быть сохранены в plaintext только при явном opt-in `SUMMARY_ENCRYPTION_DISABLED=true`; в production этот флаг игнорируется. Session-end handoff использует отдельный контракт от runtime compaction: первый нужен для следующей сессии, второй — для продолжения текущей сессии после forced reset chain.
• Метрики расхода AI для сравнения baseline/после-оптимизации логируются на сервере единым форматом: для обычного чата, chips, notifications, runtime compaction и session-end summary/profile update источник истины — `usage` из OpenAI Responses API плюс размер фактически отправленного `input` и краткий request-log с role/length breakdown промптов. Для Realtime Voice логируются оба слоя — per-response usage из realtime `response.done` и session summary с накопленными audio seconds/audio tokens. Fallback-провайдеры (`deepseek`, `yandex`) тоже обязаны логировать хотя бы request/latency/inputChars/outputChars и usage, если провайдер его возвращает. Любые продуктовые выводы об экономии должны опираться на эти логи, а не на UI-оценки.
...
• Capacitor API baseURL: в native используется правило из `app/plugins/api.ts` — в dev приоритет у `window.location.origin` сохраняется только для внешнего dev-server origin. Если приложение загружено из локального bundle (`http://localhost` / `127.0.0.1`), API должен идти в `NUXT_PUBLIC_API_SERVER_URL` (apiBase), а не в origin WebView.
...
• Картинки медитаций (covers/backgrounds) версионируются по content‑hash в имени (`*.{hash}.webp`), портретные варианты используют тот же hash (`*-portrait`/`portrait-*`), карта переименований хранится в `scripts/meditation-image-map.json`, обновление БД — `server/infrastructure/db/update-meditation-image-paths.ts`. В `resolveMediaUrl` добавлен runtime‑резолв: если путь из БД содержит устаревший хеш (например `08de5010`), он подменяется на актуальный (`ed61a3eb`) по карте — это устраняет 404 на проде при рассинхроне БД и CDN. Инвариант этой карты: target-файл обязан реально существовать в `public/meditations/**`, иначе фронт начнёт стабильно строить 404 даже при исправном CDN.
...
• В layout `default.vue` фон сцены отображается на всех страницах с этим layout, кроме `/meditations?trackId=...`, где используется `backgroundPath` самого трека. Дыхательные практики, включая конструктор `/breath-practices/custom`, визуально используют выбранный `scene` background так же, как остальные экраны приложения; route-проверка для `/breath-practices/:slug` остаётся только в логике приглушения audio сцены на реальном плеере. Панорама фона упрощена до одного слоя с диагональным движением, увеличенной длительностью и меньшим масштабом для снижения нагрузки.
...
• Android push (важно): для Android отправляются **data-only** сообщения. Нативный сервис `MentalaMessagingService` сам строит системное уведомление (title/body/image из `data`) и привязывает `contentIntent`, чтобы tap/deeplink работал одинаково в фоне, в фореграунде и на холодном старте. Событие параллельно прокидывается в Capacitor через `PushNotificationsPlugin.sendRemoteMessage(remoteMessage)`, поэтому JS-слой может обновлять UI/аналитику без отдельного `LocalNotifications` дубля. В интент обязательно кладём `google.message_id`, чтобы `PushNotificationsPlugin` эмитил `pushNotificationActionPerformed` на холодном старте.
• Android manifest-инвариант для push: в APK должен остаться ровно **один** обработчик `com.google.firebase.MESSAGING_EVENT` — `MentalaMessagingService`. Сервисы `com.capacitorjs.plugins.pushnotifications.MessagingService` и `com.google.firebase.messaging.FirebaseMessagingService` удаляются через `tools:node="remove"`, иначе доставка становится недетерминированной: часть foreground push попадает не в кастомный сервис и не показывает системное уведомление.
...
• Следующий этап унификации push-навигации: существующие `deepLink` / `navigation` / `data.action` сохраняются как rollout-compatible transport fields, но архитектурно должны подчиняться одному typed target из `.docs/unified_navigation_tz.md`, чтобы совпасть с chat navigation и рекомендациями на экранах привычек/терапии.
• Unified navigation v1 реализован поверх typed target в `shared/navigation/index.ts`: `AppNavigationTarget` стал каноническим контрактом для chat chips, push payload и programmatic CTA, а `deepLink` / `navigation` / `data.action` остались только transport/compat слоями.
• Server-side deterministic navigation слой вынесен отдельно от LLM: `server/application/navigation/chat-navigation.service.ts` строит action chips по явному запросу пользователя и по entry-context (habit/therapy/SOS), а prompt suggested chips теперь генерирует только текстовые реплики; routing knowledge не хранится в prompt.
• Client-side execution унифицирован через `useAppNavigation` + `useAppNavigationStore`: chat, push и CTA на экранах привычек/терапии вызывают один executor, который сначала применяет entitlement-gate, затем открывает target route, а при блокировке поднимает глобальный `FeaturePaywallModal` через `GlobalNavigationPaywall.vue`.
• Route guard `feature-access.global.ts` больше не делает вредный redirect на `/` для закрытых маршрутов. Он определяет typed target из route, открывает глобальный paywall и либо abort navigation, либо уводит в безопасный родительский раздел (`/practices`, `/breath-practices`, `/therapy`, `/habits`) вместо немого телепорта на главную.
...
• `PRO` (399₽): всё из Basic + текстовый AI-чат с лимитом `100 мин/нед` + полная библиотека медитаций + все дыхательные практики + AI-уведомления.
• `Premium` (899₽): всё из PRO + текстовый AI-чат «безлимит*» + персональный стиль AI-напоминаний + создание/управление своими практиками, где `*`= обязательная серверная fair-use защита.
...
• Для первого выбора «Способ создания» действует entitlement-aware дефолт: если у пользователя доступен `notifications.text_source_ai`, новые `notification_preferences`и UI fallback в`NotificationSettingsPage`стартуют с`textSource=ai`; без доступа стартовым остаётся `templates`. Это правило применяется и при автосоздании preferences для новых кастомных привычек/тем.
...
• Генерация чипов: сервис `suggested-chips.service.ts` делает один LLM-запрос без retry, без quality-gate и без серверного переопроса модели при «слабом» результате. Всё, что вернула модель, нормализуется best-effort и может быть показано пользователю; если запрос чипов завершился ошибкой, UI просто не получает блок чипов. Навигационные action-chips при этом по-прежнему строятся детерминированно server-side, а LLM генерирует только короткие text-chip'ы. Для экономики chips-prompt обязан быть существенно уже основного chat-prompt: максимум `3` чипа, диалоговый контекст режется до `4` последних реплик по `70` символов, ответ ассистента режется до `300` символов перед prompt-builder и ещё до `260` внутри prompt, onboarding-вектор даётся одной короткой строкой, отдельная строка `Тема:` не отправляется, а strict json_schema не должен тащить поля action/params, если action-chip всё равно создаются вне модели.
...
• В development CORS и origin-check используют объединение `DEV_ALLOWED_ORIGINS` + локальных defaults (`http://localhost`, `http://127.0.0.1`, `http://localhost:3000/3001`, `127.0.0.1:3000/3001`) и нормализуют origins (без хвостового `/`), чтобы native bundle на localhost и лендинг на `3001` стабильно работали с API на `3000`.
...
• Контент тарифов на лендинге синхронизирован с продуктовой матрицей (`Basic 0 ₽`, `PRO 399 ₽`, `Premium 899 ₽`).
...
• **CI/CD и деплой лендинга**: при пуше в `main`/`dev` workflow (`deploy-prod.yml`, `deploy-dev.yml`) выполняют **статический экспорт** лендинга (`pnpm landing:generate`) и деплой статики на сервер: rsync в `/var/www/landing/releases/<id>/`, атомарное переключение symlink `current`, хранение последних 5 релизов. `RELEASE_ID` формируется как `<run_id>-<run_attempt>`, а `.well-known/***` исключён из `rsync --delete`. При очистке старых релизов (`rm -rf`) перед удалением выполняется `chmod -R u+w`, а сам `rm` обёрнут в `|| true`, чтобы не падать на permission-denied для `.well-known/apple-app-site-association` и `assetlinks.json` (могут быть созданы с другими правами). Лендинг отдаётся Nginx (на хосте или в контейнере), Traefik маршрутизирует `mentala.app` на статику. Подробности — `.docs/landing_static_deploy_tz.md`.
...
🌐 Лендинг — локализация i18n (план, 4 марта 2026)
