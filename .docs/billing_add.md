...
- **Целевые платформы:** \* **Web & Android:** Полная интеграция виджета внутри интерфейса.
- **iOS (App Store):** Редирект во внешний браузер для соблюдения правил Apple.
...
| **iOS**     | **Внешний браузер** | Веб-страница | **Запрещено** использовать WebView. Только системный браузер. |
...
- **Если iOS native:** используется внешний браузер через flow внешней web-сессии.
- Технически это делается через backend-команду создания external-session и переход на одноразовый `consumeUrl`.
- Пользователь не должен проходить повторную ручную авторизацию.

---

### 5. Ответ на вопрос про Safari и внешний браузер
...
1. **Текст на кнопке:** Вместо «Купить Premium за 499р» используйте «Управление подпиской» или «Личный кабинет».
2. **Информационное сообщение:** Рядом с кнопкой можно написать: _«Настройка тарифного плана и управление платежами доступны в веб-версии сервиса»_.
3. **Отсутствие цен:** В идеале в iOS-версии вообще не должно быть цифр с ценами, если нет кнопки «Apple In-App Purchase». Просто описание функций Premium.
