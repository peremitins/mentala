...
- Streak на главной берётся из materialized state `user_streaks`, а не пересчитывается на лету из 30 последних дней. Старые поля `/api/today.streak.current/best/week` сохраняются; новые клиенты также получают `status`, `repair`, `pausedSince`, `notice`, `weekDetails`. В UI repair-квота не показывается как отдельная метрика; пользователь видит только мягкое сообщение, если серия была сохранена. Экран `/streak` использует `GET /api/streak`, ручная пауза и возврат идут через `POST /api/streak/pause` и `POST /api/streak/resume`.
