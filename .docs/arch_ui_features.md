...
- Раздел находится внутри практик и использует продуктовую рамку самонаблюдения, а не медицинской проверки. В UI основной термин — «Оценка состояния», отдельные элементы называем «оценками»; не использовать формулировки про диагноз, лечение или расстройство.
- Активные оценки: `anxiety_check_v1` / «Оценка тревоги» (валидированная шкала **GAD-7**) для сада `calm_anxiety_30` / «Спокойствие», `self_compassion_scs_sf_v1` / «Оценка внутреннего критика» (валидированная шкала **SCS-SF**) для сада `self_kindness_21` / «Внутренний критик», `relationships_boundaries_v1` / «Оценка границ и общения» для сада `relationships_21` / «Отношения» и `burnout_cbi_v1` / «Оценка выгорания и перегрузки» (валидированная шкала **CBI**) для сада `burnout_21` / «Выгорание». Вопросы и подсчёт валидированных шкал остаются максимально близкими к исходному виду, а внутренний источник авторских оценок не показывается в UI. В UI результат подаётся как самонаблюдение: без ИИ, без клинических claims и без слов «диагноз»/«расстройство», заголовки шкал в интерфейсе не показываются. Самодельный `self_kindness_v1` удалён — сад использует SCS-SF.
...
- Приложение является portrait-only на мобильных устройствах: PWA manifest задаёт `orientation: portrait`, runtime пытается вызвать `screen.orientation.lock('portrait')`, а для обычных браузерных вкладок, где iOS Safari/Chrome могут не разрешить lock, используется глобальный mobile-landscape fallback overlay вместо перестройки интерфейса в landscape.
...
- Дневной лимит новых шагов Roadmap динамический: первые два локальных дня активной программы — 3 шага в день, дальше — 2 (`DAILY_STEP_LIMIT_SCHEDULE` в `retention-timezone.ts`). День считается от `user_programs.startedAt` в таймзоне пользователя; при старте новой программы onboarding-буст включается заново. Клиент берёт актуальное значение из `programDailyLimit.dailyStepLimit` (`GET /api/today`) и ничего не хардкодит.
