...
export function computeGenerationConfigHash(config: GenerationConfig): string {
  ...
  const normalizedCustomPrompt = (config.customPromptNotification || '')
    .trim()
    .toLowerCase();

  if (normalizedCustomPrompt.length > 0) {
    // Добавляем пожелания только если они реально заданы, чтобы не ломать старые хеши
    normalized.customPromptNotification = normalizedCustomPrompt;
  }
