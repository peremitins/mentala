...
export async function chatViaProvider(params: {
  ...
  return provider.chat({ messages: params.messages, model: params.model })
}
...
export async function chatWithFallback(params: {
  ...
  model?: string
  messages: Array<{ role: 'system' | 'user' | 'assistant'; content: string }>
}): Promise<{ content: string; model?: string; provider: LlmProviderPort['id'] }> {
  ...
  for (const providerId of order) {
    ...
    try {
      const result = await provider.chat({ messages: params.messages, model: params.model })
      return { content: result.content, model: result.model, provider: providerId }
    } catch (e: any) {
      lastError = e
...
export function estimateCostUSD(params: {
  ...
  const map = config.llm.openai.pricingUSDPerMTok as Record<string, { in: number; out: number }>
