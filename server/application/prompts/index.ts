...
export const suggestedChipsSystemPrompt = `Ты — генератор вариантов реплик пользователя (suggested replies) для продолжения диалога.
...
Пиши от лица пользователя: "я", "мне", "у меня", "меня".
Обращайся к ассистенту как к помощнику ("ты"/"вы"), а не как к клиенту.
...
Если в ответе ассистента звучит рекомендация медитации/практики — допускается 1 action‑chip для открытия раздела медитаций.`;
...
- Запрещены терапевтические вопросы к собеседнику (например: "Что тебя беспокоит?", "Какие у тебя триггеры?").
- Разрешены вопросы к ассистенту и ответы на его сообщение (например: "Можешь объяснить...", "У меня так бывает...").
...
const suggestedChipsUserTemplate = `Контекст диалога:
{{dialog_context}}
...
Если тема указана — минимум один чип должен явно упоминать её, даже если разговор ушёл в сторону.
Важно: чипы — это реплики пользователя, а ассистент — терапевт/помощник.
Если в ответе ассистента есть рекомендация медитации — добавь action‑chip для открытия раздела медитаций.
Сгенерируй до {{max_chips}} чипов.`;

export const suggestedChipsRetryHint = `В прошлой попытке были повторы или слишком похожие формулировки.
...
Обязательно: минимум 2 чипа короче 40 символов и один чип с упоминанием основной темы (если она есть).`;
...
export function buildSuggestedChipsUserPrompt(params: {
  ...
  const base = renderTemplate(suggestedChipsUserTemplate, {
    dialog_context: params.dialog_context || 'Нет контекста',
    assistant_answer: params.assistant_answer || 'Нет ответа',
    recent_chips: params.recent_chips || 'Нет',
    primary_topic: params.primary_topic || 'Нет',
