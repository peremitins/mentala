...
export default defineEventHandler(async (event) => {
  try {
    ...
    const tokensIn = Math.ceil(parsed.messages.reduce((s, m) => s + m.content.length, 0) / 4)
    ...
    if (estimated > config.llm.limits.maxRequestUSD) {
      ...
      return { error: true, message: 'Estimated cost too high for single request', estimated }
    }
