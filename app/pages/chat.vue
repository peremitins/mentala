...
// Тикер времени для обновления live-счётчика длительности сессии.
// Нужен, потому что getter sessionDurationSeconds использует Date.now(),
// которое НЕ реактивно — без тикера индикатор зависал бы на значении
// из последнего сообщения до следующей активности.
...
// Живая длительность сессии в секундах: зависит от tickerNow → пересчитывается
// каждую секунду. Используем в UI вместо getter'а стора, чтобы индикатор и
// disabled-статус кнопки обновлялись даже без активности (ключевое для
// 4-минутного порога eligibility).
const liveSessionDurationSeconds = computed(() => {
  if (!chat.sessionStartedAt) return 0;
  const startMs = new Date(chat.sessionStartedAt).getTime();
  const nowMs = chat.lastActivityAt
    ? Math.max(new Date(chat.lastActivityAt).getTime(), tickerNow.value)
    : tickerNow.value;
...
// Page-level eligibility — дублирует логику стора, но зависит от tickerNow.
// Нужно, чтобы при простое UI кнопки "Завершить сессию" всё равно активировалась
// ровно через N минут после старта, а не ждала следующего сообщения.
// Пороги берутся из констант стора — в dev они 1/60s/1, в prod 5/240s/3.
const isEligibleForSummary = computed(() => {
  if (chat.userMessagesCount >= FORCE_MIN_MESSAGES) {
    return true;
  }

  return (
    chat.userMessagesCount >= DEV_MIN_MESSAGES &&
    liveSessionDurationSeconds.value >= DEV_MIN_DURATION_SECONDS &&
    chat.qualifyingUserMessagesCount >= DEV_MIN_QUALIFYING
  );
});

// Динамическая подсказка для кнопки "Завершить сессию":
// показывает текущие счётчики eligibility, чтобы пользователь понимал, чего не хватает.
...
const isFinishingSession = ref(false); // Защита от повторного клика "Завершить сессию"
const showSessionFinishedModal = ref(false); // Модалка «Сессия завершена» после ручного завершения
...
onMounted(async () => {
  // Тикер для реактивного обновления длительности сессии: eligibility
  // кнопки "Завершить сессию" зависит от времени, а не только от state.
  // Без тикера computed не пересчитается между сообщениями.
  ...
  // Обработка legacy-параметра lockedFeature (приходит с paywall-редиректа).
  ...
  // Чистим legacy query-параметр `screen` из URL — раньше он различал
  // welcome/chat экраны, сейчас чат всегда открыт.
  ...
  // Если сообщения уже есть — скроллим к последнему.
  ...
  // На холодном старте сначала пробуем восстановить незавершённую сессию с сервера.
  // Если restore успешен, приветственный auto-start запускать нельзя — иначе получим
  // конфликт между реальной историей и новым assistant greeting.
  ...
  // Иначе — автоматически стартуем диалог от ассистента (ТЗ редизайна п.1:
  // чат сразу активен, без лишнего welcome-баннера).
  scrollToBottom('auto');
...
// При уходе со страницы чата немедленно завершаем therapy-сессию —
// останавливаем счётчик биллинга. endTherapySession использует keepalive-fetch
// и retry, поэтому запрос уйдёт максимально надёжно.
// Restore при возврате работает через therapy_session_messages (не зависит от endedAt).
onBeforeUnmount(() => {
  // Останавливаем тикер eligibility — иначе утечка после ухода со страницы.
  if (tickerInterval) {
  ...
  // Останавливаем таймер биллинга немедленно.
