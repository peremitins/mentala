<template>
  <button
    type="button"
    class="group relative flex w-[78vw] min-w-[240px] max-w-[240px] flex-col overflow-hidden rounded-3xl text-left transition-all duration-300 hover:opacity-70 focus:outline-none focus:ring-2 focus:ring-primary/60 sm:w-72"
  ...
  >
    ...
    <div class="relative z-10 flex h-full flex-col gap-2 p-4">
      <div class="flex items-start justify-between gap-2">
        <div class="space-y-1">
          <!-- <p
            class="text-[10px] uppercase tracking-[0.08em] text-white/70 drop-shadow"
          >
            {{ practice.goal }}
          </p> -->
          <h4 class="text-md font-semibold text-white leading-snug drop-shadow">
            {{ practice.title }}
          </h4>
          <p class="line-clamp-2 text-xs text-white/85 drop-shadow">
