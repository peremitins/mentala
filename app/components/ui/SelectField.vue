<template>
  ...
  >
    <SelectTrigger
      class="inline-flex min-w-[160px] items-center justify-between rounded-full h-10 px-4 text-sm bg-white/10 border border-white/15 backdrop-blur shadow-[0_4px_16px_rgba(0,0,0,.25)]"
    >
    ...
    <SelectPortal>
      <SelectContent
        ...
        class="z-50 min-w-[200px] max-h-64 overflow-auto rounded-2xl border border-white/15 bg-white/10 border border-white/15 backdrop-blur shadow-[0_8px_24px_rgba(0,0,0,.45)] p-1"
      >
        <SelectViewport class="p-1">
          <template v-for="(group, gi) in normalizedOptions" :key="gi">
            <SelectLabel v-if="group.label" class="px-3 py-1 text-xs opacity-70">{{
              group.label
            }}</SelectLabel>
            <SelectGroup>
              <SelectItem
              ...
              >
                ...
                />
                <SelectItemText class="truncate pl-2">{{ opt.label }}</SelectItemText>
              </SelectItem>
            </SelectGroup>
          </template>
        </SelectViewport>
    ...
    <div class="inline-block">
      <PopoverRoot v-model:open="open">
        <PopoverTrigger as-child>
          <button
            type="button"
            class="inline-flex min-w-[200px] items-center justify-between rounded-full h-10 px-4 text-sm bg-white/10 border border-white/15 backdrop-blur shadow-[0_4px_16px_rgba(0,0,0,.25)]"
          >
            <span class="truncate text-left">{{ selectedLabels.join(', ') || placeholder }}</span>
            <IconChevronDown class="h-4 w-4" />
          </button>
        </PopoverTrigger>
        <PopoverPortal>
...
const flatOptions = computed<Option[]>(() => normalizedOptions.value.flatMap((g) => g.items))
